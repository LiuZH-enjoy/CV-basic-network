...
class Transformer(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        t_self_mask = create_trg_self_mask(target_size,
    ...
    def get_encoder(self, x):
        i_mask = create_pad_mask(x, self.src_pad_idx)
        enc_output = self.encode(x, i_mask)
        return enc_output, i_mask

    def get_decoder(self, x, enc_output, i_mask):
        t_mask = create_pad_mask(x, self.trg_pad_idx)
        target_size = x.size()[1]
        t_self_mask = create_trg_self_mask(target_size,
                                           device=x.device)

        return self.decode(x, enc_output, i_mask, t_self_mask, t_mask)

if __name__ == '__main__':
    import numpy as np
    model = Transformer(64, 64, src_pad_idx=0, trg_pad_idx=0)
